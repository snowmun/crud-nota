<template>
  ...
  import noteUserServices from '@/services/note/noteUserService'
  ...
  export default {
    ...
    methods: {
      ...
      async getNoteUser(){
        ...
        await noteUserServices.noteUserList(idUser)
          .then(({code , data}) => {
              if (code == 200 && data != []){
                  this.noteList = data.nota.map((nota, index) => ({...nota, index: index + 1}))
                  this.$toasted.success('vista de la nota cargada')
              }else{
                  this.$toasted.error('El usuario no registra notas')
              }
          })
          .catch(() => {
